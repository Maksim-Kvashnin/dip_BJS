...
**Внимание!** В проекте присутствует eslint, который выполняется перед коммитом.
При коммите, если возникает ошибка и коммит не выполняется, откройте логи git'а и исправьте ошибки. Коммиты не будут выполняться, если линтер не будет успешно проверять код.

Для ручного запуска линтера используйте `npm run lint`. Используйте эту команду для статического анализа кода.
